...
/*   Updated: 2017/11/02 14:15:33 by fhuang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	*draw_image(void *args)
{
	...
	{
		c.real = helper->offset.x / helper->fractal.zoom
			+ helper->fractal.abscissa.min;
		helper->offset.y = 0;
		while (helper->offset.y < helper->range.y)
		{
			c.imaginary = helper->offset.y / helper->fractal.zoom
				+ helper->fractal.ordinate.min;
			helper->f(helper, z, c);
			++helper->offset.y;
		}
		++helper->offset.x;
