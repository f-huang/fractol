...
/*   Updated: 2017/11/02 17:11:40 by fhuang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	convert_color(t_rgb *rgb, t_mlx_img *mlx_img, t_fractal fractal, int i)
{
	t_rgb			*colors;
	double			coef;
	...
	if (!coef)
		coef = 255;
	colors[F_BW] = (t_rgb) { .r = 255 - (10 * i), .g =  255 - (10 * i), .b =  255 - (10 * i) };
	...
	colors[F_RANDOM_1] = (t_rgb) { .r = 250 * coef, .g = 250 * coef, .b = 250 * i };
		...
		t_mlx_img *mlx_img, t_fractal fractal, t_offset offset, int i)
{
	...
	{
		convert_color(rgb, mlx_img, fractal, i);
	}
